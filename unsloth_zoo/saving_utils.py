...
def merge_and_overwrite_lora(
    ...
    def resize_cached_embed(filename, save_directory, embedding_size):
        ...
        if old.shape[0] > embedding_size:
            print(f"[Unsloth] Truncating {embed_key} from {old.shape[0]} -> {embedding_size} vocab rows in {filename}")
            new = old[:embedding_size].clone()
        else:
            print(f"[Unsloth] Expanding {embed_key} from {old.shape[0]} -> {embedding_size} vocab rows in {filename}")
